...
public class PingOnline {
    ...
    private static boolean oneGetServerName; // 初次获取服务器名称
    ...
    public void sendPlayerPacket(String playerName) {
        try {
            InetAddress address = InetAddress.getByName(ip);
            String message = "LoginSequence-Player " + playerName;
            byte[] sendData = message.getBytes();
            DatagramPacket packet = new DatagramPacket(sendData, sendData.length, address, port);
            socket.send(packet);
            plugin.getLogger().info("Sent player login packet to remote server for: " + playerName);
        } catch (IOException e) {
            plugin.getLogger().log(Level.WARNING, "Failed to send player login packet: " + e.getMessage());
        }
    }
